<?php
    session_start();

    if (!isset($_COOKIE['status']) || $_COOKIE['status'] !== 'true') {
        header("Location: ../../Login/View/login.php");
        exit();
    }

    if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'Admin') {
        echo "Access denied!";
        exit();
    }

    if (!isset($_SESSION['user'])) {
        echo "User data not found!";
        exit();
    }
    require_once '../../DB/dbUser.php';
?>

<html>
<head>
    <title>Healthcare Management System</title>
    <link rel="stylesheet" href="../Asset/cssAdminN.css">
</head>

<body>
<script src="../Asset/adminList.js"></script>
<script>initList('Nurse');</script>
<header class="topbar">
    <div class="logo">
        <img src="../../Icons/logo.png" alt="Logo">
        <span>Healthcare Management System</span>
    </div>

    <div class="topbar-right">

        <div class="icon">
            <form action="../../Profile/View/profile.php" method="post" class="logout-form">
                <button type="submit" class="icon-btn">
                    <img src="../../Icons/profile.svg" alt="Profile">
                </button>
            </form>
            
        </div>
        <div class="icon">
            <form action="../../Logout/Controller/logout.php" method="post" class="logout-form">
                <button type="submit" class="icon-btn">
                    <img src="../../Icons/logout.svg" alt="Logout">
                </button>
            </form>
        </div>
    </div>
</header>

<div class="container">

    <aside class="sidebar">
        <ul>
            <li ><a href="adminDashboard.php">Home</a></li>
            
            <li class="menu-title">Manage</li>
            <ul class="submenu">
                <li><a href="doctorList.php">Doctor</a></li>
                <li class="active"><a href="nurseList.php">Nurse</a></li>
                <li><a href="receptionistList.php">Receptionist</a></li>
                <li><a href="patientList.php">Patient</a></li>
            </ul>

            <li><a href="bills.php">Bills</a></li>
            <li><a href="../../User_Activity/View/userActivity.php">User Record</a></li>
        </ul>
    </aside>

    <main class="content">

        <h1>Nurse List</h1>
        <div id="successMessage" style="color:green;"></div>

        <?php
            $con = connection();
            $nurses = [];

            $query = "select name,age,dob,email,password,mobile,speciality 
                    from users where role='Nurse'";

            $result = mysqli_query($con, $query);

            if ($result && mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    $nurses[] = $row;
                }
            }
        ?>

        <div class="nurse-layout">

            <!-- LEFT: Nurse List -->
            <div class="nurse-list">
                <table class="nurse-table">
                    <thead>
                        <tr>
                            <th>Name</th>
                            <th>Age</th>
                            <th>DOB</th>
                            <th>Email</th>
                            <th>Password</th>
                            <th>Mobile</th>
                            <th>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($nurses as $nrs) { ?>
                        <tr>
                            <td><?php echo $nrs['name'] ?></td>
                            <td><?php echo $nrs['age'] ?></td>
                            <td><?php echo $nrs['dob'] ?></td>
                            <td><?php echo $nrs['email'] ?></td>
                            <td><?php echo $nrs['password'] ?></td>
                            <td><?php echo $nrs['mobile'] ?></td>
                            <td>
                                <button type="button" onclick='selectItem(
                                    "<?php echo $nrs['name'] ?>",
                                    "<?php echo $nrs['age'] ?>",
                                    "<?php echo $nrs['dob'] ?>",
                                    "<?php echo $nrs['email'] ?>",
                                    "<?php echo $nrs['mobile'] ?>"
                                )'>Select</button>
                            </td>
                        </tr>
                        <?php } ?>
                    </tbody>
                </table>
            </div>

            <!-- RIGHT: nurse Management -->
            <div class="nurse-manage">
                <form action="../Controller/checkValidAll.php" method="post" id="nurseForm">
                    <input type="hidden" name="role" id="role" value="Nurse">

                    <input type="hidden" name="action" id="action">
                    <input type="hidden" name="email_key" id="email_key">

                    <table class="nurse-manage-table">
                        <tr>
                            <th colspan="2">Nurse Management</th>
                        </tr>

                        <tr>
                            <td>Name</td>
                            <td><input type="text" name="name" id="name" disabled><div id="nameError" class="error" style="color:red;"></div></td>
                        </tr>

                        <tr>
                            <td>Age</td>
                            <td><input type="number" name="age" id="age" disabled><div id="ageError" class="error" style="color:red;"></div></td>
                        </tr>

                        <tr>
                            <td>DOB</td>
                            <td><input type="date" name="dob" id="dob" disabled><div id="dobError" class="error" style="color:red;"></div></td>
                        </tr>

                        <tr>
                            <td>Email</td>
                            <td><input type="email" name="email" id="email" disabled><div id="emailError" class="error" style="color:red;"></div></td>
                        </tr>

                        <tr>
                            <td>Password</td>
                            <td><input type="password" name="password" id="password" disabled><div id="passwordError" class="error" style="color:red;"></div></td>
                        </tr>

                        <tr>
                            <td>Mobile</td>
                            <td><input type="text" name="mobile" id="mobile" disabled><div id="mobileError" class="error" style="color:red;"></div></td>
                        </tr>
                        <tr>
                            <td colspan="2">
                                <div class="btn-group">
                                    <button type="button" onclick="addItem()">Add</button>
                                    <button type="button" onclick="editItem()">Edit</button>
                                    <button type="button" onclick="deleteItem()">Delete</button>
                                    <button type="button" onclick="saveItem()" id="saveBtn" disabled>Save</button>
                                </div>
                            </td>
                        </tr>
                    </table>
                </form>
            </div>
        </div>
    </main>
</div>

</body>
</html>

